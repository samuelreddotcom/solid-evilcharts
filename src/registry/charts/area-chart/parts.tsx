...
/** Declares the resting point marker for the enclosing `<Area>`. */
export const Dot = createMarker<"dot", DotProps>("dot");

/** Declares the hovered/active point marker for the enclosing `<Area>`. */
 ...
 *
 * `<Dot>` / `<ActiveDot>` are read from each `<Area>`'s own children. Because a
...
export function collectConfig(children: unknown): CollectedConfig {
  ...
  for (const slot of filterSlots(slots, "area")) {
    const props = slotProps<AreaProps>(slot);
    ...
    areas.push({
      ...
      // <Area> defaults to a DASHED stroke, where <Line> defaults to solid.
      // A deliberate upstream divergence, preserved.
